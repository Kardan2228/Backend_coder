...
class Contenedor {
  ...
  async _readFileOrCreateNewOne() {
    try {
      ...
      error.code === "ENOENT"
        ...
        : console.log(
            `Código de error: ${error.code} Hubo un error al intentar leer el archivo ${this._filename}`
          );
    }
  }
  ...
  async _createEmptyFile() {
    fs.writeFile(this._filename, "[]", (error) => {
      error
        ? console.log(error)
        : console.log(`El archivo ${this._filename} fue creado, ya que no existía`);
    });
  }
  ...
  async getById(id) {
    ...
    try {
      ...
      console.log(
        `Código de error: ${error.code} Hubo un error al intentar leer el artículo con id: (${id})`
      );
    }
  }
  ...
  async deleteById(id) {
    try {
      ...
      console.log(
        `Código de error: ${error.code} Hubo un error al intentar eliminar el producto con ID (${id})`
      );
    }
  }
    ...
    if(this._validateKeysExist(newData)){
      try {
        ...
        if (objectIdToBeUpdated) {
          ...
          console.log(`El ID ${id} no existe en el archvio`);
          return null;
        }
  ...
  async save(object) {    
    if(this._validateKeysExist(object)) {
      try {
        ...
        console.log(
          `Código de error: ${error.code} Hubo un error al intentar guardar el producto`
        );
      }
    } else {
      return false;
  ...
  async deleteAll() {
    try {
      ...
      console.log(
        `Se presentó el error (${error.code}) al intentar borrar todos los productos`
      );
    }
  }
